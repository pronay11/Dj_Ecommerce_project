...
from product.models import CartItem, Item, Category, WishListItem
...
class RemoveFromWishView(APIView):
    def get(self, request, wish_item_id):
        item = get_object_or_404(WishListItem, pk=wish_item_id)
        item.delete()
        return Response({'status': 'ok'}, status=status.HTTP_200_OK)
